...
class mem_ctrl_scoreboard #(
    ...
    function void report_phase(uvm_phase phase);
        ...
        `uvm_info("SB_REPORT", "===== MEM_CTRL SCOREBOARD REPORT =====",  UVM_LOW)
        `uvm_info("SB_REPORT", $sformatf("Writes issued:        %0d", write_count),         UVM_LOW)
        `uvm_info("SB_REPORT", $sformatf("Reads issued:         %0d", read_count),          UVM_LOW)
        `uvm_info("SB_REPORT", $sformatf("B  responses:         %0d", b_count),             UVM_LOW)
        `uvm_info("SB_REPORT", $sformatf("R  responses:         %0d", r_count),             UVM_LOW)
        `uvm_info("SB_REPORT", $sformatf("Invalid WR addr (M5): %0d", addr_invalid_writes), UVM_LOW)
        `uvm_info("SB_REPORT", $sformatf("Invalid RD addr (M5): %0d", addr_invalid_reads),  UVM_LOW)
        `uvm_info("SB_REPORT", $sformatf("Data mismatches:      %0d", data_mismatches),     UVM_LOW)
